({
    ...
    saveAnnulato : function(component,event,helper){
        var detailEvent = component.get("v.detailEvent");
        delete detailEvent.DurationInMinutes;
        delete detailEvent.ActivityDateTime;
        delete detailEvent.ActivityDate;
            console.log("detailEvent" + JSON.stringify(detailEvent));
            ...
            if (response.getState() == "SUCCESS") {
                ...
                if (resultat.erreur==false) {
                    ...
                    component.set("v.detailEvent",resultat.detailEvent);
                    helper.navigate2Event(component,detailEvent.Id);
                    //$A.get('e.force:refreshView').fire();
                    } else {
                       // this.showToast("Appuntamento Già Annullato",'error');
                       this.showToast("Si è verificato un errore nel salvataggio: "+resultat.message,'error');
                        console.log("ERROR MESSAGE "+resultat.message)
                    }
                }
                else{
                console.log(response.getError());
    ...
    navigate2Event : function(component,eventId){
        var pageReference={    
            "type": "standard__recordPage",
            "attributes": {
                "recordId": eventId,
    ...
    aggAppuntamentoConfirm :function(component,event,helper){
        component.set('v.confirmMessage',true);
        component.set('v.messageError',' Confermi l\'aggiornamento dell\'appuntamento?');
    },
    ...
    getValueList3: function (component,event,helper) {
        ...
        if(value==='No parametri minimi'){
            ...
            //ATTENZIONE AL FATTO CHE LA DISPOSITION TORNA IN MODO ASINCRONO CAZZO!!!!!
            this.getDisposition(component,code);
            ///IL RESTO DEL CODICE DOVREBBE GIRARE QUANDO LA DISPOSITION E' TORNATA!!!
            ...
            if(nuovoStato=='Appuntamento modificato'){
                ...
                component.set("v.ModifyEvent",false);
               // this.aggiornaAppuntamento(component);
               this.aggAppuntamentoConfirm(component);
            }else{
                component.set("v.ModifyEvent",false);
                this.annulAppuntamento(component,event,helper);
            }
    ...
    getValueMotivazione : function (component,event){
        ...
        if(nuovoStato=='Non presentato/Annullato'){
           ...
           this.aggAppuntamentoConfirm(component,event);
        }else{
            component.set("v.ModifyEvent",true);
        }
    },
    ...
    controleMinutesAndHoure : function (component){
        ...
        var prodottoId =component.get("v.detailEvent.Product__c");
        var resultat;
        var listTimes;
        var listHour=[];
        ...
        if(detailEvent.hasOwnProperty('Product__r')){
            ...
            if(pruduct.hasOwnProperty('RSS_External_Id__c')){
                if(detailEvent.Product__r.RSS_External_Id__c!=null){
                    if(detailEvent.Product__r.RSS_External_Id__c=='1079' || ((parentProduct!=null)&&('6297'==parentProduct.RSS_External_Id__c))){
