({
    ...
    continuCreaAppuntamento1: function (component,event,helper){
        ...
        var phoneValidity = component.find('telAbit').get('v.validity').valid;
        var telefonoCellulareValidity = component.find('telCell').get('v.validity').valid;

        if((phone1 && !phoneValidity) || telefonoCellulare && !telefonoCellulareValidity){
            this.showToast("Errore nel formato dei numeri di telefono", "error");
            return;
        }
            ...
            if (response.getState() === "SUCCESS") {
                ...
                if (resultat) {
                    if(resultat.erreur==true){
                        this.showToast("Errore: Non è stato possibile fissare un appuntamento.","error");
                    ...
                    if(resultat.prodotto==false){
                        ...
                        console.log('Lancio evento appuntamentoFissatoEvt');
                        let evento = $A.get("e.c:appuntamentoFissatoEvt");
                        if(evento){
                            evento.setParams({'caseId': caseId});
                            evento.setParams({'accountId': cliente.Id});
                            evento.fire();
                        }
                ...
                helper.showToast(JSON.stringify(response.getError()));
    ...
    getValueCliente: function(component) {
        ...
        var testCondition= document.getElementById("checkbox-unique-id-71");
            ...
            if(selectPI.selectedIndex){
                ...
                // setto il prodotto selezionato (prodotto figlio) per gestire dopo autonomiPlus
                var  selectProdDett = document.getElementById("ProductDettaglio");
                if(selectProdDett!=null && selectProdDett.selectedIndex){ 
                    productDettaglio = selectProdDett.options[selectProdDett.selectedIndex].value;
                    var idProductDettaglio = productDettaglio.split('/');
                    if (idProductDettaglio!=null && idProductDettaglio[0] != null && idProductDettaglio[2] != null) {
                        component.set("v.idProdottoSelezionato",idProductDettaglio[0]);
                        component.set("v.externalProdottoSelezionato",idProductDettaglio[2]);
                    }
                }    
            }else{
    ...
    controleMinutesAndHoure : function (component){
        ...
        var listHour=[];
        var filialeAppunta = component.get("v.filialeAppunta");
        var prodottoId = component.get("v.newEvent.Product__c");
        var filialeId = component.get('v.filialeId');
        var filialeName;
        if(filialeId!=""){
            filialeName=filialeAppunta.Name;
    ...
    getValueFotocopieAllegare : function(component){
        ...
        var externalProdottoSelezionato= component.get('v.externalProdottoSelezionato');
        var newEvent= component.get("v.newEvent");
        ...
        if(productExternal!=null){
            if(productExternal=='1079' || '6297'==externalProdottoSelezionato){
    ...
    filtraPerLatitudineLongitudine : function(elemLista,elemMappa) {
        if (elemLista!=null && elemMappa!=null && elemMappa.location!=null && 
            elemMappa.location.Latitude == elemLista.latitudine &&  elemMappa.location.Longitude == elemLista.longitudine) {
                return true;
        }
        return false;
    },
    filtraMappaMarkers : function(mapMarkers,lista) {
        var mapFiltrata = [];
        if (mapMarkers!=null && lista!=null) {
            for (var i =0; i<mapMarkers.length;i++) {
                for (var j =0; j<lista.length;j++) {
                    if (this.filtraPerLatitudineLongitudine(lista[j],mapMarkers[i])) {
                        mapFiltrata.push(mapMarkers[i]);
                    }
                }
            }
        }
    ...
    filtraFilialiAutonomiPlus : function(component, lista, productId,mapMarkers){
        console.log('filtraFilialiAutonomiPlus lista ', lista);
        console.log('filtraFilialiAutonomiPlus productId ', productId);
        var action = component.get('c.filtraFilialiAutonomiPlus'); 
        action.setParams({ "lista": lista, 
                            "productId": productId }); 
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                var result = response.getReturnValue();
                console.log('listFilialiAutonomi result ', result);
                if (result) {
                    component.set("v.showListFiliale",true);
                    //console.log('### resultat ', JSON.stringify(resultat));
                    console.log('### filtraFilialiAutonomiPlus listFiliale ', result);
                    mapMarkers  = this.filtraMappaMarkers(mapMarkers,result);
                    console.log('### filtraFilialiAutonomiPlus mapMarkers ', mapMarkers);
                    component.set("v.listFiliale",result);
    ...
    checkAutonomiPlus : function(component, event){
        console.log('checkAutonomiPlus inizio');
        // nuovo controllo per autonomi plus
        var  selectPI = document.getElementById("prodottoInteresse");
        if(selectPI.selectedIndex){ 
            prodottoInteresse = selectPI.options[selectPI.selectedIndex].value;
            var idProdottoSelez = prodottoInteresse.split('/');
            if (idProdottoSelez==null || (idProdottoSelez[2] != '6296')) {
                // non è stato selezionato il prodotto autonomi plus: OK esco
                console.log("passaAutonomiPlus idProdottoSelez :"+idProdottoSelez[2]);   
                return true;
            }      
            var  selectProdDett = document.getElementById("ProductDettaglio");
            if(selectProdDett.selectedIndex){ 
                ProductDettaglio = selectProdDett.options[selectProdDett.selectedIndex].value;
                var idProductDettaglio = prodottoInteresse.split('/');
                if (idProductDettaglio!=null || idProductDettaglio[0] != null) {
                    console.log("passaAutonomiPlus idProductDettaglio "+idProductDettaglio[0]);   
                    return true;    
                }
                  // non è stato selezionato il prodotto filgio autonomi plus: errore
            }
            return false;
        }
        return true;    
