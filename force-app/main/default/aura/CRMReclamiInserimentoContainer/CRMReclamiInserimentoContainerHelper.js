...
({
    ...
    checkDettagli:function(cmp){
        ...
        // correzione forzata Dotti
        if(cmp.find('coda_dettaglio')==null ||  cmp.find('coda_dettaglio').get('v.codaSelezionata')==null || cmp.find('coda_dettaglio').get('v.codaSelezionata').DeveloperName==null){
            this.showToast(cmp, 'Errore','error', 'Non è stata selezionata la coda');
            res = false;
        }
        return res;
    },
    ...
    checkMittenti:function(cmp){
        ...
        var mittenti=cmp.get('v.mittentiList');
        var isPrincipale=false;
        var isMittentiCompleted=true;
        for(var i=0; i<mittenti.length;i++){
            if(mittenti[i].Principale__c) { 
                isPrincipale=true;
            }
            if (!this.campoValorizzato(mittenti[i].Tipologia__c)) {
                    isMittentiCompleted=false;
            }
        }
        if(!isPrincipale){
             this.showToast(cmp, 'Errore','error', 'Per salvare il reclamo è necessario inserire un mittente principale');
            return false;
        } 
        if(!isMittentiCompleted){
            this.showToast(cmp, 'Errore','error', 'Per salvare il reclamo tutti i mittenti devono aver valorizzato: tipologia');
           return false;
       } 
       
        return res;
    },
    campoValorizzato: function(campo) {
        console.log('campoValorizzato campo:'+campo);
    ...
    salvaReclamoHelper:function(cmp,helper){
        ...
        var comunicazione = cmp.find('coda_dettaglio').get("v.presaInCarico");
        var email =  cmp.find('coda_dettaglio').get("v.comunicazioneEmail");
        var tel = cmp.find('coda_dettaglio').get('v.numSMS');
        ...
        action.setParam('comunicazione',comunicazione);
        action.setParam('email',email);
        action.setParam('tel',tel);
...
//        var attrList=['dettagliOutputObj','clienteSelezionatoContainer','praticaSelezionataContainer',
        var attrList=['dettagliOutputObj','clienteSelezionatoContainer','praticaSelezionataContainer','comunicazionePresaInCarico',
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    isEmailValida : function (mail){
        var x  = (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(mail));    
        console.log(x + ' Email non Valida');
        return x;
    },
    
    isNumeroTelefonoValido : function(cmp,numero){
       var x =  (/^3[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]?[0-9]?$/.test(numero)); 
        console.log(x + ' Numero Valido');
        return x;
    }
