...
*/
 ...
 // AV TODO rimettere                 cmp.get("v.parent").nascondiClessidra();
    ...
    selectClienteHelper: function (cmp, event, helper) {
        console.log('Show pratiche: '+cmp.get("v.showPratiche"))
        if (cmp.get("v.showPratiche")) {
 // AV TODO rimettere           cmp.get("v.parent").mostraClessidra();
            ...
            action.setCallback(this, function (resp) {
                ...
                if (resp.getState() == 'SUCCESS') {
                    ...
                    if (resp.getReturnValue()['clienti'].length == 1) {
                        ...
                        /*
                        //Gestione Per Fusione Anagrafica - 
                        //Fix necessaria per far partira onCLienteSelected nel childComponent nel caso in cui ci siano più clienti
                        var listSelId = [];
                        // preselezione dell'unico cliente della lista
                        listSelId.push(resp.getReturnValue()['clienti'][0]['codCliente']);
                        cmp.set('v.idOCSClienteSelezionato', listSelId);
 ...
 // AV TODO rimettere              cmp.get("v.parent").nascondiClessidra();
            ...
            $A.enqueueAction(action);
        }//Fix necessaria per far partira onCLienteSelected nel childComponent nel caso di Fusione Anagrafica
        else{
            //console.log("Get row:"+JSON.stringify(event.getParam("selectedRows")));
            cmp.set('v.OCSClienteSelezionato', event.getParam("selectedRows")[0]);
        }
