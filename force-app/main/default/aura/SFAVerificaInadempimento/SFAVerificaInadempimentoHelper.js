    ...
    saveAttachments: function(cmp) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var caseList = cmp.get('v.caseList');
                var selectedCase = cmp.get('v.selectedCase');
                var newCaseList;
                for(var i = 0; caseList.length < i; i++)
                {
                   if(caseList[i].id != selectedCase.id)
                   {
                       newCaseList.push(caseList[i]);
                   }
                }
                cmp.set('v.caseList',newCaseList);
                cmp.set('v.selectedCase',null);
                cmp.set('v.refreshView',false);
                cmp.set('v.refreshView',true);
            }
            else if (state === 'ERROR') {
                var errors = response.getError();
                if (errors) {
