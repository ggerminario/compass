({
    init : function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                helper.checkLoadingCSV(component,event,helper);
            }
            else if (state === "ERROR") {
    ...
    cancel : function(component,event,helper){
        ...
        helper.resetUpload(component,event); 
    },
    ...
    confirm : function(component,event,helper){
        var files = component.get("v.fileToBeUploaded");
        var bypassConfirmUpload =  component.get("v.bypassConfirmUpload");
        if (files && files.length > 0) {
            component.set("v.bypassConfirmUpload",false);
			var file = files[0][0];
            console.log('confirm file:'+file);
            component.set('v.disableSelect', false);
		    helper.confirmUpload(component,file,helper);
		} else if (bypassConfirmUpload) {
            component.set('v.bypassConfirmUpload',false);
            component.set('v.disableSelect', false);
		    helper.confirmUpload(component,null,helper);
        }
    },
