({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (resultCheck!=null && resultCheck.error!=null && resultCheck.error!='') {
                    ...
                    component.set('v.errormsg', 'Errore: '+resultCheck.error);
    ...
    parseRow: function (component,row,campo) {
        if (row!=null) {
            var rowField = row.split(";");
            var headerList =  component.get('v.headerList');
            if (headerList!=null && rowField.length>=headerList.length) {
                for (var i=0;i<headerList.length;i++) {
                    if (headerList[i]!=null && headerList[i]==campo) {
                        return rowField[i];
                    }
                } 
            }
                ...
                if (state === "SUCCESS") {
                    ...
                    if(listError!=null && listError.length > 0) {
                        ...
                        for (var i = 0;i<listError.length;i++) {
                            ...
                            errRec.hiddenCSSClass = 'hiddenRow';
                            ...
                            // numero pratica /carta
                            var numero = this.parseRow(component,errRec.row,'PRATICA');
                            if (numero!='') {
                                errRec.numero = numero;
                            } else {
                                errRec.numero =  this.parseRow(component,errRec.row,'CARTA');
                            }
                             // filiale
                            errRec.filiale = this.parseRow(component,errRec.row,'DIP');
                            // cliente
                            errRec.cliente =  this.parseRow(component,errRec.row,'CLIENTE');
                            // dealer (agente/convenzionato/cod_dealer)
                            var dealer = this.parseRow(component,errRec.row,'COD_CV');
                            if (dealer!='') {
                                errRec.dealer = dealer;
                            } else {
                                dealer = this.parseRow(component,errRec.row,'COD_AG');
                                if (dealer!='') {
                                    errRec.dealer = dealer;
                                } else {
                                    errRec.dealer =  this.parseRow(component,errRec.row,'COD_DEALER');
                                }
                            }
                            // esito
                            errRec.esito =  this.parseRow(component,errRec.row,'COD_ESITO');
                            errRec.descEsito =  this.parseRow(component,errRec.row,'DESCR_ESITO');
                            if (errRec.rowCSSClass == 'errorRow') {
                                // aggiungo alla tabella solo i record errati
    ...
    setTableError : function(component){
        ...
        component.set('v.columnsError', [
            ...
            {label: 'Errore', fieldName: 'description', type: 'text', cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: 'Pratica/Carta', fieldName: 'numero', type: 'text', initialWidth: 120, cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: 'Filiale', fieldName: 'filiale', type: 'text', initialWidth: 80, cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: 'Dealer', fieldName: 'dealer', type: 'text', initialWidth: 120, cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: 'Nome Cliente', fieldName: 'cliente', type: 'text', initialWidth: 300, cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: 'Esito', fieldName: 'esito', type: 'text', initialWidth: 80, cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: 'Descrizione Esito', fieldName: 'descEsito', type: 'text', cellAttributes: { class: { fieldName: 'rowCSSClass' }}},
            {label: '', fieldName: 'row', type: 'text', initialWidth: 2, cellAttributes: { class: { fieldName: 'hiddenCSSClass' }}},
    ...
    checkLoadingCSV: function(component,event,helper) {
        // controllo se ci sta in cache un csv in elaborazione non troppo vecchio
        var action = component.get("c.checkExistingLoading");
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                var result = response.getReturnValue();
                console.log('checkLoadingCSV result:'+result);
                if(result!=null){
                    // se ci sta elbaorazione in cache
                    console.log('result.tipoFile:'+result.tipoFile);
                    console.log('result.prodotto:'+result.prodotto);
                    component.set("v.tipo_lista",result.tipoFile); 
                    component.set("v.tipo_lista_selezionata",result.tipoFile);   
                    this.getSelectProdottoByTipoFile(component,event);               
                    component.set("v.prodotto",result.prodotto);
                    component.set("v.prodotto_selezionato",result.prodotto);
                    component.set("v.filename",result.filename);
                    component.set("v.timestamp",result.timestamp);
                    component.set("v.showSpinner",true);
                    component.set("v.showUpload",true);
                    component.set("v.showMain",true);
                    component.set("v.bypassConfirmUpload",true);
                    component.set('v.headerList', result.header);
                    this.statusJob(component, event, helper);
                }
    ...
    deleteExistingLoading :  function(component,event) {
    },
    resetUpload : function(component, event) {
        // ripristina situazione pre-caricamento file
        component.set('v.showSectionError', false);
        component.set('v.showConfirm', false);
        component.set('v.disableSelect', false);
        component.set('v.numOK', 0);
        component.set('v.numError', 0);
        // pulische cache backe-end
        var action = component.get("c.removeExistingLoading");
        action.setParams({ 
            fileName : component.get('v.filename'),
            timestamp : component.get('v.timestamp')
        });
