({
    SearchHelper: function(component, event) {
        ...
        // if (codicedealerValue == '' || campagnaValue == '') {
        //     this.showToast('nessun record trovato', 'Error')

        // }
        if (codicedealerValue == '' && campagnaValue == 'Selezionatutte') {
            this.getSearchCampagna(codicedealerValue, campagnaValue, component);
        } else if (codicedealerValue != '' && campagnaValue != '') {
            this.getSearchCampagna(codicedealerValue, campagnaValue, component);
        } else if (campagnaValue != '' && codicedealerValue == '') {
            this.getSearchCampagna(codicedealerValue, campagnaValue, component);
        }
    },

    getSearchCampagna: function(codicedealerValue, campagnaValue, component) {
    ...
    deleteSelected: function(component, event, selctedRec) {
        var spinner = component.find("csvSpinner");
        $A.util.toggleClass(spinner, "slds-hide");
        console.log("selctedRecHelper", selctedRec);
        var action = component.get("c.updateCase");
        action.setParams({ "listIdcase": selctedRec });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state == "SUCCESS") {
                this.showToast('attività eliminata correttamente..');
