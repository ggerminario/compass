({
    init:function(cmp,event,helper){
        ...
        if(cloned){
            ...
            if(cmp.get('v.canDelete') && !cmp.get('v.isDisabled')){
    ...
    deleteFile:function(cmp,event,helper){
        ...
        if(fileList.length==1 && !cmp.get('v.canDeleteAll')){
            ...
            console.log('Deleting file: ' + row.Name);

            var rows = cmp.get('v.fileList');
            ...
            cmp.set('v.fileList', rows);
            
            helper.hideSpinner(cmp, event);
            /*
            var list=[];
            var fileName = row.Name;
            var idParent = row.ParentId;
            var idDocument = row.Description;
