({
  ...
  submitDetails: function(component, event, helper) {
    ...
    if (dispo != null && dispo != "") {
      ...
      var closeCaseVar;
      if(!caseObject.CampaignId__c){
        closeCaseVar = true;
      } else {
        closeCaseVar = component.get('v.closedCase');
      }
      ...
      if (state === "SUCCESS") {
        ...
        if (!results.error) {
          if (results.data != null) {
            ...
            //ricerca id della disposition "Chiede di essere richiamato"
            var idDisp = results.data.find(function(el){
              if(el.Name == 'Chiede di essere richiamato') return el;
            })
            if(idDisp){
              component.set("v.idChiamato",idDisp.Id);
            }
          }
        } else {
          helper.showToast(
