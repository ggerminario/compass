({
    ...
    searchDealer: function (component, event, helper) {
        ...
        if(CodDealer!=null && CodDealer!=''){
            ...
            action.setCallback(this, function(data) {
                let retVal = data.getReturnValue();
                let Dealer = retVal;
                console.log('DEALER TROVATO '+JSON.stringify(retVal))
                if(Dealer!=null){
    ...
    create : function(component, event, helper) {
        ...
        component.set("v.FileUpload", files.length > 0);
        
        var mapAttached = new Map();
        
        files.forEach(function (item, index) {
            mapAttached[item.Name] = item.Body;
        });

        component.set("v.MapAttach", mapAttached);
