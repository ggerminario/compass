({
    handleUpdateCampaign: function(cmp, event) {
        ...
        var listValue = cmp.get('v.lstItem');
        var tab = [];
        var element = '';
        
        for (var val in listValue) {
            tab.push(listValue[val].label);
            element = tab.join(',');
        }
        ...
        cmp.set("v.newCampaign.UAF_DatiAggiuntivi__c", element);
    ...
    getCampaignById: function(component, selectedCampaign) {
        ...
        action.setCallback(this, function(a) {
            ...
            for (var i in list) {
                ...
                if(datiAggiuntiviField){
                    var splitdatiAggiuntivi = datiAggiuntiviField.split(',');
                    for( var j in splitdatiAggiuntivi){
                    datiAggiuntiviList.push({
                        label: splitdatiAggiuntivi[j],
                        value: splitdatiAggiuntivi[j]
                    });
                }
                }
    ...
    handleChange: function(component, event, helper) {
        ...
        if (txtVal == '') {
            ...
            var matchedIndex = AllRowsList.map(function(obj) { return obj.label; }).indexOf(txtVal[0]);
            component.set("v.valCheckbox", matchedIndex);
            var valcheckbox = component.get("v.valCheckbox");
            console.log('valcheckbox', valcheckbox);
