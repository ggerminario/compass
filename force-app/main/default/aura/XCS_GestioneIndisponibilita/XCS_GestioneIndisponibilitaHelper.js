({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
				...
				if(regionList.length === 0){
					helper.showToast("Errore: Non è stato possibile trovare le filiali competenza.",'error');	
					return;
				} 	
				regionList.reverse();
	...
	setDeskTable: function(cmp, event, helper) {
		...
		if(!cmp.get("v.filialeSelected")) {
			helper.showToast("Errore: Selezionare una filiale per proseguire.",'error');
			spinner.decreaseCounter();
			return;
		}
	...
	updateAppointmentList: function (cmp, event, helper,newAppointmentList,oldAppointmentList) {
        ...
        action.setCallback(this, function(response) {
            ...
            var state = response.getState();
            if (state === "SUCCESS") {
				if(oldAppointmentList && oldAppointmentList.length > 0)
