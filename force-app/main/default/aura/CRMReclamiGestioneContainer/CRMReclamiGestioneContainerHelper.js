({
    ...
    settaDatiPraticaAggiuntivi: function(cmp, event){
        ...
        if(societa != 'Compass'){
            cmp.set("v.praticaSelezionataReady", true);
            return;
        } 
        ...
        action.setCallback(this,function(resp){
            if(resp.getState()=='SUCCESS'){
                if(resp.getReturnValue() == null || resp.getReturnValue()== undefined){
                    ...
                    cmp.set("v.praticaSelezionataReady", false);
    ...
    recuperaAssicurazione:  function(cmp,event){
        ...
        action2.setCallback(this,function(resp){
            if(resp.getState()=='SUCCESS'){
                cmp.set("v.refreshReclamiInserimentoContainerPadre", false);
                cmp.set("v.infoPraticaSelezionata", resp.getReturnValue());  
                 cmp.set("v.refreshReclamiInserimentoContainerPadre", true);
            }
    ...
    aggiornaDataScadenzaHelper : function(cmp,event){
       
        var action=cmp.get('c.isEditableDataScadenza');
        var societa = cmp.get('v.campiCase').Referenced_Company__c;
        action.setParams({
            societa: societa,
            tipoReclamo: cmp.get('v.tipoDettagliContainerTipo')
        });
        action.setCallback(this,function(resp){
            if(resp.getState()=='SUCCESS'){
                cmp.set('v.isEditDataScadenza', resp.getReturnValue());
            }
            this.hideSpinner(cmp);
        });
        this.showSpinner(cmp);
...
//        $A.enqueueAction(action);
    ...
    getHasEditAccess:function(cmp){
        ...
        actionEditAccess.setCallback(this,function(resp){
            if(resp.getState()=='SUCCESS'){
                ...
                console.log('getHasEditAccess v.hasEditAccess:'+resp.getReturnValue());
            }
            this.hideSpinner(cmp);
        });
        this.showSpinner(cmp);
    ...
    buildPageBodyAttributes:function(cmp,inputObj,helper){
        ...
        var pratica;
        if(cliente.pratiche != null && cliente.pratiche.length > 0) cmp.set('v.praticaSelezionataContainer',cliente.pratiche[0]);
        else if(cliente.praticheFuturo != null && cliente.praticheFuturo.length > 0) cmp.set('v.praticaSelezionataContainer',cliente.praticheFuturo[0]);
    ...
    checkDettagli:function(cmp){
        ...
        if(!res){
            cmp.set('v.errorMessage',"Controllare i seguenti campi nel tab 'Dettagli': "+cmp.get('v.dettagliMessage'));
        }
        return res;
    },
    ...
    makeDettagli : function(cmp,event,helper){
        ...
        res['commissioniSelection'] = cmp.get('v.commissioniSelection');
        console.log('commissioniSelection:'+res['commissioniSelection']);
        res['provvAccSelection'] = cmp.get('v.provvAccSelection');
        res['premioAssSelection'] = cmp.get('v.premioAssSelection');
        res['varieSelection'] = cmp.get('v.varieSelection');
        res['speseLegaliSelection'] = cmp.get('v.speseLegaliSelection');
    ...
    makeDettagliDouble : function(cmp,event,helper){
        ...
        res['commissioniValue'] = cmp.get('v.commissioniValue');
        res['premioAssValue'] = cmp.get('v.premioAssValue');
        res['provvAccValue'] = cmp.get('v.provvAccValue');
        res['varieValue'] = cmp.get('v.varieValue');
        res['speseLegaliValue'] = cmp.get('v.speseLegaliValue');
